...
class Listpublications
{
    ...
    private GpRessourcesRepository $formulesRepository;

    public function __construct(GpRessourcesRepository $formulesRepository, BoardRepository $boardRepository, PostEventRepository $postEventRepository, OffresRepository $offreRepository, PostRepository $postRepository)
    {
    ...
    public function listAllPotins(): array
    {
        $taball=[];
    ...
    public function listPublications($key): array
    {
        $tabpost=[];
        $taboffre=[];
        $posts=$this->postRepository->findAllByKey($key);
        /** @var Post $post */
        foreach ($posts as $post){
            if(!$post->isDeleted()){
        ...
        $offres=$this->offreRepository->ListOffresByKey($key);
        /** @var Offres $offre */
    ...
    public function findAllEventsBeforeOneWeek($listboard): array
    {
        $taball=[];
    ...
    public function listBoardByTime(): array
    {
        $tabhit=[];
        $tabloc=[];
        $listboard=[];
    ...
    public function listPublicationsAll($listboard): array
    {
        $taball=[];
        $posts=$this->postRepository->findlast();
        ...
        $offres=$this->offreRepository->findlast();
        /** @var Offres $offre */
        foreach ($offres as $offre){
    ...
    public function listPublicationsThanCityArround($locate, $listboard): array
    {
        ...
        /** @var Post $post */
        foreach ($posts as $post){
            $taball[$post->getCreateAt()->getTimestamp()]=['board'=>$this->boardRepository->findWbByKey($post->getKeymodule()), "post"=>$post];
        }
        $offres=$this->offreRepository->findAroundlastBycity($locate);
        /** @var Offres $offre */
        foreach ($offres as $offre){
            $taball[$offre->getCreateAt()->getTimestamp()]=['board'=>$this->boardRepository->findWbByKey($offre->getKeymodule()), "offre"=>$offre];
        }
        krsort($taball);
        return $taball;
    }
    ...
    public function listBoardByHitsThanCity($city): array
    {
        ...
        $sortBoard=[];
        $tabidwebsite=[];
        $wbs=$this->boardRepository->findWebsiteOfLocate($city);
        foreach ($wbs as $wb) {
            $tabidwebsite[$wb['id']]=$wb;
            $listboard[$wb['codesite']] = $wb;
            if( $wb['statut']) $tabloc[]=$wb;// pour identification du board de la marie locale
