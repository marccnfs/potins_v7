...
class UploadServices
{
    ...
    private $version = 'v7.2cc';
	
	// Utilisée dans la classe d'upload php et dans le code php des formulaires 
	...
	*/
	public function renameIdenticName($adresse_fichier, $incr = false, $unix = false, $stop = 0, $isfile = false)
	{
		if ($isfile || is_file($adresse_fichier))
		{
			...
			if (trim($incr) != false && $stop < 90)// le stop arbitrtaire est une mesure de sécurité au cas où...
			{
				...
				$file = preg_replace_callback('#([0-9]+$)#', function($matches) {return $matches[1]+1;}, $dernier, '1', $count);
	...
	// La fonction "cleanFileName" est utilisée dans la fonction "GetPostFile" elle-même appelée par le constructeur de la classe d'upload, pour définir $this->cleanFname
