...
class ElusController extends AbstractController // board-v5-1/back-admin/?keyboard=v5-12020test
{

    use adminsession;


    #[Route('elus-accès', name:"elus_acces")]
    public function eluOffice( BoardslistRepository $spwsiteRepository, Initbacktor $initbacktor, BoardRepository $websiteRepository,Messageor $messageor, DispatchSpaceWebRepository $dispatchSpaceWebRepository): Response
    {
        $this->denyAccessUnlessGranted('ROLE_ELUS');
...
*/
    ...
    #[Route('valid-acces-elus/{keyboard}', name:"valid-acces-elus")]
    public function majCustomers($keyboard): Response
    {
        $vartwig=$this->menuNav->templateControl(
            Links::CUSTOMER_LIST,
            'functions',
            "functions",
            'all');
        if($keyboard!=007) $this->redirectToRoute('index');
        $this->redirectToRoute('elus-accès');
