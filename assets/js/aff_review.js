$(document).ready(function() {
    ...
    let btnpost = $('#post-review'),
        infreview=$('#infreview'),
        post=infreview.data('post'),
        gpreview=infreview.data('gp'),
        rw= infreview.data('rw'),
        contenthtml,
        incontenthtml=infreview.data('content'),
        type=infreview.data('type'),
        editor = $('#editor'),
        datatoAjax=false;
    ...
    let routemanagereview ='/ajax/addreview/add/'+gpreview+'/'+rw,
  ...
  function initToolbarBootstrapBindings() {
   ...
   initToolbarBootstrapBindings();
    ...
    tabfield.titrereview.on('change', function () {
        if (tabfield.titrereview.val()) {
            propp(true)
        }
    });
    ...
    let startloadernews = function () {
        let fd = new FormData();
        fd.append('titre', tabfield.titrereview.val());
        fd.append('soustitre', tabfield.soustitre.val());
        fd.append('fiche', contenthtml);
        fd.append('type', type);
        fd.append('post', post);
        fd.append('gp', gpreview);
        fd.append('idrw', rw);
        fd.append('file64', datatoAjax);

        $.ajax({
            type: 'post',
            url: routemanagereview,
            data: fd,
            processData: false,
            contentType: false,
            beforeSend: function () {
