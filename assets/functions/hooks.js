...
import {ApiError, jsonFetch, typesenseFetch} from './api.js'
...
/**
 * Hook faisant un appel typsense et flash en cas d'erreur / succès
 *
 * @param {string} params
 * @return {{data: Object|null, tpsfetch:tpsfetch, loading: boolean, done: boolean}}
 */
export function useTypeSenseFetchOrFlash (params) {
  const [state, setState] = useState({
    loading: false,
    data: null,
    done: false
  })
  const tpsfetch = useCallback(
      async (localParams) => {
        setState(s => ({ ...s, loading: true }))
        try {
          const response = await typesenseFetch(localParams)
          setState(s => ({ ...s, loading: false, data: response, done: true }))
          return response
        } catch (e) {
          if (e instanceof ApiError) {
