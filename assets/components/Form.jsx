...
export function Field ({
  ...
  if (FieldComponent === FieldEvent) {

    return (
        <div className={`form-textearea_all ${wrapperClass}`} ref={ref}>
          <FieldComponent {...attr} />
          {children && (
                {children}
          )}
  ...
  if (FieldComponent === FieldMsgmob) {

    return (
        <div className={`msg-texta ${wrapperClass}`} ref={ref}>
          <FieldComponent {...attr} />
          {children && (
              {children}
          )}
...
function FieldEvent (props) {
  return <textarea {...props} />
}

function FieldMsgmob (props) {
  return <textarea {...props} />
}
...
export function FetchFormEvent ({
  ...
  const handleSubmit = async e => {
    ...
    if(changeImg){
      formData.imgdata=img;
    }else{
      formData.imgdata=false;
    }
    formData.change={'edit':edit,'img':changeImg,'tabdate':changeTabDate,'adress':changeAdress,'partner':changePartner};
    formData.now=JSON.stringify(tabdate);
    formData.adresse=JSON.stringify(adress);
    formData.partners=JSON.stringify(tabpartners);
    formData.event=event?event:false;
    formData.board=board;
    formData.potin=potin?potin:false;
...
export function FetchFormOffre ({
    data = {},
    children,
    action,
    className,
  ...
  const handleSubmit = async e => {
    ...
    formData.change={'edit':edit,'img':changeImg,'tabdate':changeTabDate};
    formData.now=JSON.stringify(tabdate);
    //formData.adresse=JSON.stringify(adress);
    //formData.partners=JSON.stringify(tabpartners);
    formData.offre=offre?offre:false;
    formData.board=board;
    //formData.potin=potin?potin:false;
 ...
 * @param onSuccess Fonction appelée en cas de retour valide de l'API (reçoit les données de l'API en paramètre)
 */
export function FetchFormEntity ({
data = {},
children,
action,
className,
 ...
 * Formulaire Ajax
 */
export function FetchFormMedia ({
      data = {},
      children,
      action,
      className,
  ...
  const handleSubmit = async e => {
    ...
    setState({ loading: true, errors: [] })
    const form = e.target
   // const formData = { ...data} --> ce serait plus adapté mais modifier alors dans le controller et le service
    const formData = { ...data, ...Object.fromEntries(new FormData(form)) }
    formData.adresse=JSON.stringify(adress);
...
export function FetchFormControl ({
     data = {},
     children,
     action,
     className,
